...
void setup()
{
	...
	for (int x = 0; x<100; x++)
	{
		gyroOffset[0] += MPU9150_readSensor(MPU9150_GYRO_XOUT_L, MPU9150_GYRO_XOUT_H) / 131;
		gyroOffset[1] += MPU9150_readSensor(MPU9150_GYRO_YOUT_L, MPU9150_GYRO_YOUT_H) / 131;
		gyroOffset[2] += MPU9150_readSensor(MPU9150_GYRO_ZOUT_L, MPU9150_GYRO_ZOUT_H) / 131;
		delay(2);
	}
	gyroOffset[0] /= 100;
	gyroOffset[1] /= 100;
...
void loop()
{
	...
	// the temperature.32768
	
	//double dT = ((double)MPU9150_readSensor(MPU9150_TEMP_OUT_L, MPU9150_TEMP_OUT_H) + 12412.0) / 340.0;
	// Magnetometer
	int mx = MPU9150_readSensor(MPU9150_CMPS_XOUT_L, MPU9150_CMPS_XOUT_H);
	int my = MPU9150_readSensor(MPU9150_CMPS_YOUT_L, MPU9150_CMPS_YOUT_H);
	int mz = MPU9150_readSensor(MPU9150_CMPS_ZOUT_L, MPU9150_CMPS_ZOUT_H);

	// Gyro Rates
	int p = MPU9150_readSensor(MPU9150_GYRO_XOUT_L, MPU9150_GYRO_XOUT_H);
	int q = MPU9150_readSensor(MPU9150_GYRO_YOUT_L, MPU9150_GYRO_YOUT_H);
	int r = MPU9150_readSensor(MPU9150_GYRO_ZOUT_L, MPU9150_GYRO_ZOUT_H);
	...
	acc[0] = (double)ax / 16384;
	acc[1] = (double)ay / 16384;
	acc[2] = (double)az / 16384;
	double a[3] = { 0 };

	gyro[0] = (double)p / 131 - gyroOffset[0];
	gyro[1] = (double)q / 131 - gyroOffset[1];
	gyro[2] = (double)r / 131 - gyroOffset[2];
	...
	//gyroY /=  131;
	//angle += 100/1000 * gyro[2];

	/*Serial.print(dT / 1000000, DEC);
	Serial.print("  ");
	Serial.print(angle);
	Serial.print("  ");
	Serial.print(acc[0]);
